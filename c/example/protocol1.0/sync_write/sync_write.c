...
int main()
{
    ...
    while (1)
    {
        ...
        dxl_addparam_result = GroupSyncWrite_AddParam(group_num, DXL1_ID, dxl_goal_position[index], 2);
        ...
        dxl_addparam_result = GroupSyncWrite_AddParam(group_num, DXL2_ID, dxl_goal_position[index], 2);
        if (dxl_addparam_result != true)
        {
        ...
        GroupSyncWrite_TxPacket(group_num);
        if ((dxl_comm_result = GetLastTxRxResult(port_num, PROTOCOL_VERSION)) != COMM_SUCCESS)
            PrintTxRxResult(PROTOCOL_VERSION, dxl_comm_result);

        // Clear syncwrite parameter storage
        GroupSyncWrite_ClearParam(group_num);
